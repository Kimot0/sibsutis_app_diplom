...
suspend fun<T> safeApiCall(apiCall: suspend()-> T): Requests<T> {
    return try {
        Requests.Success(apiCall())
    } catch (throwable: Exception) {
        when (throwable) {
            is HttpException -> Requests.Error(
                (mapError(
                    throwable
                ))
            )
            is CancellationException -> Requests.Error(
                mapError(
                    throwable
                )
            )
            is JsonDataException, is JsonEncodingException -> Requests.Error(
                mapError(
                    throwable
                )
            )
